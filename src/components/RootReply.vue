<template>
  <div class="list-item reply-wrap" :id="'r-'+reply.reply.reply.replyId">
    <div class="user-face">
      <a :href="'#/user/'+reply.reply.user.uid" target="_blank">
        <img :src="reply.reply.user.face?reply.reply.user.face:'/static/akari.jpg'" alt="">
      </a>
    ...
    <div class="con">
      <div class="user">
        <a :href="'#/user/'+reply.reply.user.uid" target="_blank"
           class="name vip-red-name">{{reply.reply.user.nick}}</a>
        <!-- <a href="#" target="_blank"><i class="level l4"></i></a> -->
      </div>
      <p v-html="replyContent" class="text"></p>
      <div class="info">
        ...
        <operation :root="is_root"
                   :index="index"
                   :r_uid="reply.reply.user.uid"
                   :reply_id="reply.reply.reply.replyId"
                   v-on:delRootReply="delRootReply"
        >
      ...
      <div class="reply-box">
        ...
        <sub-reply v-for="(m_reply,index) in (view_more?reply.replies.slice(0,10):reply.replies.slice(0,3))"
                   :show_reply_box="show_reply_box"
                   :index="index"
                   :reply="reply"
                   :m_reply="m_reply"
                   :key="m_reply.reply.replyId"
                   v-on:toggleRB="toggleRB"></sub-reply>
        ...
        <div v-if="(reply.reply.totalSize>3) && !view_more" @click="viewMore" class="view-more">共<b>{{reply.reply.totalSize}}</b>条回复,
          <a class="btn-more" data-pid="809139213">点击查看</a></div>
      </div>
      <padding-box v-show="reply.reply.totalSize>10" v-on:changeSubRepliesPage="changeSubRepliesPage"
                   :sub_replies="reply.replies" :view_more="view_more" :root_reply="reply"></padding-box>
      <post-reply v-if="show_reply_box" :show_reply_box="show_reply_box" v-on:updateSubComment="updateSubComment"
                  v-on:postReplySuccess="postReplySuccess" :episode_id="episode_id"
                  :reply_info="replyInfo"></post-reply>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      delRootReply(index){
        console.log("delRootReply in RootReply.vue");
        this.replies.splice(index,1);
      },
      delSubReply(index){
        console.log("delSubReply in RootReply.vue");
      }
    },
    computed: {
      replyContent() {
        //替换所有的换行符
        let string = this.reply.reply.reply.content;
